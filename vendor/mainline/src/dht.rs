...
impl DhtBuilder {
    ...
    /// Use an existing UDP socket for outbound K-RPC packets.
    ///
    /// The DHT actor still binds its own receive socket. Embedders that share
    /// a public UDP port with another protocol can therefore preserve the
    /// public source port for DHT requests and responses while forwarding
    /// inbound packets to the actor's private endpoint.
    pub fn outbound_socket(&mut self, socket: Arc<UdpSocket>) -> &mut Self {
