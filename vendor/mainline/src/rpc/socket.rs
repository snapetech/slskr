...
impl KrpcSocket {
    ...
    fn send(&mut self, address: SocketAddrV4, message: Message) -> Result<(), SendMessageError> {
        let socket = self.outbound_socket.as_deref().unwrap_or(&self.socket);
...
mod test {
    ...
    #[test]
    fn outbound_socket_preserves_public_source_port() {
        let receiver = UdpSocket::bind((Ipv4Addr::LOCALHOST, 0)).unwrap();
        let public_socket = Arc::new(UdpSocket::bind((Ipv4Addr::LOCALHOST, 0)).unwrap());
        let public_address = public_socket.local_addr().unwrap();
        let mut client = KrpcSocket::new(&Config {
            bind_address: Some(Ipv4Addr::LOCALHOST),
            outbound_socket: Some(Arc::clone(&public_socket)),
            ..Default::default()
        })
        .unwrap();

        client.request(
            receiver.local_addr().unwrap().try_into().unwrap(),
            RequestSpecific {
                requester_id: Id::random(),
                request_type: RequestTypeSpecific::Ping,
            },
        ...
        let (_, source) = receiver.recv_from(&mut buffer).unwrap();
