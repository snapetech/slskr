...
test.describe('library ingest', () => {
  ...
  test('items_appear_in_ui_with_metadata', async ({ page, request }) => {
    ...
    // Wait for the nav sidebar to load after login. Browse may live inside a
    // collapsed nav group, so it's attached but not visible until clickNav
    // opens that dropdown below.
