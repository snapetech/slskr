...
Status: `slskr-protocol` and `slskr-client` are production-quality: real Soulseek wire codecs, full session/peer/transfer runtime, and passing test suites. `slskr-cli` probe commands and the Proton NAT-PMP matrix scripts work end-to-end. `slskr serve` is a single-node daemon with a manual HTTP/1.1 server, bearer-token/browser-cookie auth, CSRF origin checks for mutating routes, a defined REST API surface, `/api/events` polling, and `/api/events/ws` WebSocket events. The bundled web UI is partially wired and now uses the plain WebSocket event feed instead of SignalR. Search persistence has one real default-off path (`SLSKR_PERSISTENCE_ENABLED` / `[persistence].enabled`): search create writes to SQLite and startup can hydrate `/api/searches`; transfer/message/room persistence is still incomplete. Some long-tail API endpoints remain compatibility stubs. See [REMEDIATION.md](./REMEDIATION.md) for the current cleanup plan.
...
Current gap: the workspace has the protocol/runtime/probe pieces and a working `slskr serve` daemon scaffold with a background session manager, listener ownership, peer metadata responses, startup/rescan share indexing with a state-dir cache, share catalog/root file APIs, browse/search responses, direct and indirect transfer execution/resume with chunked progress events, max-active admission control, inbound/outbound transfer allow switches, reloadable transfer records, bearer-token and same-site browser-cookie auth, cross-site mutating-request rejection, a bundled local dashboard, WebSocket event delivery, session privilege checks, projection event/stats/metrics/telemetry endpoints, user watch/stats/browse controls, room-list sync plus room join/leave/message controls, and initial install/service docs. It still needs durable app storage for transfers/messages/rooms, removal or replacement of remaining compatibility stubs, and fuller parity across browse, messages, rooms, and long-running health views.
