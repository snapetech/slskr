...
if ! rg -n 'x_forwarded_for|forwarded_header_client_ip|x_forwarded_for_client_ip' crates/slskr/src/lib.rs crates/slskr/src/http_server.rs >/dev/null; then
  printf 'rate-limit proxy policy check failed: Forwarded/X-Forwarded-For parsing is missing\n' >&2
  status=1
fi

if ! rg -n 'trusted_proxy_cidrs.*any|rate_limit_remote_addr' crates/slskr/src/lib.rs >/dev/null; then
