...
run_download_probe_optional() {
  ...
  command+=(cargo run -q -p slskr --bin slskr -- probe download-peer)
...
run_commons_download_probe_optional() {
  ...
  run_probe_optional "$scope" open-commons-browse "open commons browse proof failed; text fixture browse remains the blocking proof" "$actor_index" "$peer_user" env SLSK_BROWSE_HOST_OVERRIDE="$host" SLSK_BROWSE_EXPECTED=commons-click-track.ogg cargo run -q -p slskr --bin slskr -- probe browse-peer
  ...
  run_account_command_optional "$scope" open-commons-search "public search indexing can lag or suppress fresh daemon shares" "$actor_index" env SLSK_SOAK_LISTENER_BIND=127.0.0.1:0 SLSK_SOAK_SEARCH_QUERY=commons-click-track.ogg SLSK_SOAK_SECONDS=15 cargo run -q -p slskr --bin slskr -- soak live
}
...
wait_for_peer() {
  ...
  for _ in $(seq 1 "$attempts"); do
    ...
    stdout_file="$(mktemp)"
    stderr_file="$(mktemp)"
    local command=(cargo run -q -p slskr --bin slskr -- probe peer-address)
    if [[ "$probe_vpn_enabled" == "1" ]]; then
      local label host_ip ns_ip subnet namespace config
      mapfile -t probe_args < <(probe_netns_args "$scope")
      label="${probe_args[0]}"
...
if prepare_commons_fixtures; then
  run_logged slskr fixture-peer-smoke "$repo_root" cargo run -q -p slskr --bin slskr -- smoke fixture-peer
else
...
if [[ "$run_daemons" == "1" ]]; then
  if [[ -d "$slskr_repo" ]]; then
    ...
    if wait_for_peer slskr-to-slskr "$slskr_probe_account_index" "${!slskr_user_var}"; then
      run_probe_optional slskr-to-slskr plain-peer "public login/reset instability or daemon listener race; local peer smoke remains the blocking proof" "$slskr_probe_account_index" "${!slskr_user_var}" env SLSK_PLAIN_HOST_OVERRIDE="$slskr_host" cargo run -q -p slskr --bin slskr -- probe plain-peer
      ...
      run_probe_optional slskr-to-slskr file-transfer-peer "raw transfer token echo requires a queued transfer on slskr; real payload transfer is covered by queued download probes" "$slskr_probe_account_index" "${!slskr_user_var}" env SLSK_FILE_HOST_OVERRIDE="$slskr_host" cargo run -q -p slskr --bin slskr -- probe file-transfer-peer
