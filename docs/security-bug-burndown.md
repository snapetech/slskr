...
| Medium | Release package check | Release package verification used `--allow-dirty --no-verify`. | Fixed by running strict `cargo package --workspace`. |
...
| Medium | Frontend reverse tabnabbing | Several `_blank` opens omit `noopener`/`noreferrer`, including API-provided stream URLs (`web/src/components/Search/Response.jsx:552`, `web/src/components/Shares/SharedWithMe.jsx:146`, `web/src/components/Shares/SharedWithMe.jsx:368`). | Use a shared safe-open helper that applies `noopener,noreferrer`, restricts expected stream URL origins where possible, and add component tests. |
