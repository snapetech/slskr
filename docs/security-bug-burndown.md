...
| Medium | Web build script failure masking | `SLSKR_BUILD_WEB` in `crates/slskr/build.rs` logs npm build failures but does not fail the cargo build (`crates/slskr/build.rs:32`, `crates/slskr/build.rs:43`). | Return a build-script error or panic when the requested web build fails, and include stdout/stderr in the cargo error. |
