...
| Medium | State-file symlink handling | Transfer-state reads followed symlinks, and the shared atomic writer opened its temporary path with truncate semantics, allowing an occupied symlink to modify another file before rename. | Fixed by rejecting non-regular transfer-state paths, opening Unix reads with `O_NOFOLLOW`, creating temporary files exclusively, and testing that symlink targets remain untouched. |
