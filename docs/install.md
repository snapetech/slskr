...
On Unix, `SLSKR_STATE_DIR` must be a real directory rather than a symlink. slskr creates or tightens it to mode `0700` because it contains message, transfer, share-index, SQLite, and durable mesh identity state.

In the native/current profile, the TLS 1.3 mesh-service/private-gateway listener shares the regular Soulseek TCP listener when DHT and mesh overlay are enabled; no second TCP forward is needed. Set `SLSKR_OVERLAY_BIND` or `[listeners].overlay_bind` only when you need an explicit dedicated compatibility listener, for example `0.0.0.0:50305`. Startup fails if a required listener cannot bind or the durable certificate/key pair is missing, malformed, or inconsistent. Read `certificateSha256` from authenticated `GET /api/mesh/transport` and place that 64-character hexadecimal digest in the Pod's `gatewayCertificateSha256` policy. The effective overlay port must be reachable by Pod members; the normal HTTP API port does not carry overlay traffic.

Mainline-DHT rendezvous defaults on in the native/current profile. Override it with `SLSKR_DHT_ENABLED` / `[dht].enabled` and choose the UDP bind port with `SLSKR_DHT_PORT` / `[dht].port`; the default is `50300` and remains independent when `soulseek.listen_port` is customized. The default mesh overlay and QUIC UDP services share that UDP number, while the mesh TCP overlay shares the Soulseek TCP number. Port `0` is accepted only when DHT is disabled. Outbound-only Pod members can enable DHT without a shared public TCP overlay listener to discover pinned gateway candidates.
