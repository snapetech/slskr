...
- mesh and service-fabric HTTP status projections, plus an executable verified multi-source HTTP range swarm with pollable jobs. Frozen-format signed Soulseek capability hello/ack messages populate a bounded observed-peer registry. The BitTorrent-DHT rendezvous, TLS overlay, and JSON service fabric used by slskdN are not implemented, so current DHT/NAT/overlay responses are compatibility projections rather than a live slskdN mesh. A durable bounded content-discovery store resolves exact size plus `byteHash`, `fullFileHash`, or `fileSha256` metadata to MusicBrainz recording IDs and shadow-index peer IDs; source-less swarm requests can select slskr-local HTTP range endpoint metadata before normal preflight and SHA-256 verification.
