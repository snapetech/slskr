...
- mesh and service-fabric HTTP status projections, plus an executable verified multi-source HTTP range swarm with pollable jobs. Frozen-format signed Soulseek capability hello/ack messages populate a bounded observed-peer registry. Pinned TLS 1.3 overlay client and server paths implement bounded `SLSKDNM1` framing; the `private-gateway`, frozen `pods` List/Get/Join/Leave/PostMessage/GetMessages, aggregate-only `shadow-index` QueryByMbid/QueryBatch, and ranged `MeshContent` GetByContentId services are executable. Inbound calls require a fresh Soulseek-authenticated capability and matching peer IP; slskr peers additionally sign the overlay hello with their durable capability key and bind it to the gateway certificate, while frozen unsigned clients retain the exact capability/IP service path. Pod room `readonly`/`mirror` bindings and `soulseek-dm:` channels bridge live messages through the normal bounded Soulseek session and durable stores. A bounded Mainline BitTorrent-DHT client announces and queries the frozen primary and two backup SHA-1 rendezvous keys, publishes real status, and supplies certificate-pinned gateway endpoint candidates. Because the frozen capability wire omits the certificate pin and range URL, operators can configure bounded peer-ID/username/endpoint/pin trust records with optional range URL templates. A durable bounded content-discovery store resolves exact size plus `byteHash`, `fullFileHash`, or `fileSha256` metadata to MusicBrainz recording IDs and shadow-index peer IDs; source-less swarms can select trusted or slskr-local HTTP range endpoints, and preview tickets can fetch exact verified ranges through a trusted peer's `MeshContent` overlay.
...
- short-lived local, direct Soulseek peer, and SHA-256-pinned mesh-range audio preview byte streams
...
- `GET /api/v0/hashdb/stats`, `GET /api/v0/hashdb/entries`, `GET /api/v0/hashdb/hash/:flacKey`, and `GET /api/v0/hashdb/hash/by-size/:size`: inspect the bounded durable hash metadata registry and its audio-only share-index projection. Entry listing supports bounded `limit` and `offset` paging.
- `POST /api/v0/hashdb/hash` and `POST /api/v0/hashdb/sync/merge`: validate and merge exact-size SHA-256 metadata, including optional MusicBrainz recording IDs, into `content-discovery-state.json`. Merge batches are atomic and sequence IDs, timestamps, and use counts are server-owned.
- `POST /api/v0/virtualsoulfind/shadow-index/sync/merge` and `GET /api/v0/virtualsoulfind/shadow-index/:recordingId`: maintain and inspect the bounded recording-to-peer relation used by mesh source discovery. Source-less `POST /api/v0/multisource/swarm` requests use this relation plus operator-trusted or verified live peer metadata and still require at least two usable range sources. The overlay `shadow-index` service intentionally returns only frozen-compatible aggregate variant counts, while `MeshContent` performs the subsequent pinned exact-range byte fetch.
...
- `POST /api/v0/searches`: create a search record from JSON body `{"query":"..."}` or automation-compatible `{"searchText":"..."}`, match against the current local share snapshot, optionally persist it to SQLite, and enqueue the public-network search command when connected. Optional `target` values are `global`, `user`, `room`, or `wishlist`; user searches require `username`, room searches require `room`, and `ttl_seconds` controls the active search inactivity window (15 seconds by default, reset when a peer response arrives; scheduled wishlist searches use an explicit five-minute window).
...
- `GET /api/v0/browse`: list up to 1,024 cached browse projections with at most 10,000 file entries per user. Persisted hydration and new response merges enforce the same ceilings. Supports `q`, `status`, `limit`, and `offset` query parameters.
...
- `GET/POST /api/v0/pods`, `GET/PUT/DELETE /api/v0/pods/:podId`, and Pod member action routes: maintain bounded durable Pod definitions and membership in `pods.json`, deriving the acting peer from the current local Soulseek identity. Private Pod reads require membership; mutations require the corresponding owner/moderator or self-service authority.
- `GET/POST /api/v0/pods/:podId/channels/:channelId/messages`: maintain a bounded durable member-only channel history with stable message IDs and incremental `since` cursors in `pod-channel-messages.json`. Deleting a Pod also removes its retained messages.
- `POST /api/v0/pods/:podId/channels/:channelId/bind` and `unbind`: persist validated `readonly` or `mirror` Soulseek-room binding metadata for moderators. The live bridge joins bound Soulseek rooms, ingests room messages, sends `mirror` channel messages through the normal session command path, and projects `soulseek-dm:` channels through the durable private-message conversation.
...
- `GET/POST/PUT/DELETE /api/wishlist` item routes, `/api/wishlist/import/csv`, and release-radar/source-feed wishlist helpers: maintain up to 10,000 wishlist projections using store-owned monotonic IDs, atomic batch capacity checks, bounded/deduplicated hydration, and SQLite write-through when persistence is enabled
...
- `GET/POST/PUT/DELETE /api/collections` and collection-item routes: maintain up to 256 collections with 10,000 items each, store-owned monotonic item IDs, bounded/deduplicated SQLite hydration, reordering, and write-through when persistence is enabled
- `GET/POST/PUT/DELETE /api/share-grants` plus collection lookup/token/backfill helpers: maintain share-grant projections with SQLite write-through when persistence is enabled; token helpers return cryptographically random, explicitly ephemeral compatibility values that are not accepted as authentication credentials
...
- `SLSKR_OBFUSCATED_LISTENER_BIND` to enable and bind the type-1 obfuscated peer listener in the `native` runtime profile; the `legacy` profile has no type-1 listener
- `SLSKR_OBFUSCATED_ADVERTISED_PORT` for the public obfuscated peer port advertised to the server in the `native` profile
- `SLSKR_OVERLAY_BIND` is optional in the native/current profile. With DHT and mesh overlay enabled, the stock projection shares the regular Soulseek TCP listener with the TLS 1.3 mesh-service/private-gateway listener; the TOML equivalent is `[listeners].overlay_bind`. Set an explicit different address or port to retain a dedicated compatibility listener. The gateway creates durable restricted `overlay-certificate.der`, `overlay-private-key.der`, and `peer-capability-key.bin` identity files in the state directory, advertises `mesh_sync` with the effective port, and exposes the certificate SHA-256 fingerprint through `GET /api/mesh/transport` for Pod policy pinning.
- `SLSKR_DHT_ENABLED` enables Mainline-DHT rendezvous and defaults to enabled in the native/current profile. `SLSKR_DHT_PORT` selects the UDP bind port and defaults to `50300`, independently of a customized Soulseek TCP port; the default mesh overlay and QUIC UDP services share that number. TOML equivalents are `[dht].enabled` and `[dht].port`. The client refreshes and announces the frozen `slskdn-mesh-v1` primary/backup keys on a bounded interval; `GET /api/dht/status` reports bootstrap, routing, public-address, firewall, announce, lookup, and discovered-beacon state.
- `SLSK_OBFUSCATION` enables type-1 obfuscated listener advertisement and fallback paths in the `native` profile; it defaults to `true` there. The `legacy` profile ignores these native-only obfuscation layers and exposes no type-1 listener. In the `native` profile, `SLSK_OBFUSCATION_MODE=compatibility` is the default and dials regular peer-message, browse, and file-transfer endpoints before obfuscated endpoints. `prefer` reverses that order when `SLSK_OBFUSCATION_PREFER_OUTBOUND=true`; both modes retain the other transport as fallback. Unsupported `only` mode fails startup. Equivalent TOML keys live under `[network.obfuscation]`.
...
- `SLSKR_TRANSFER_AUTO_RETRY_ENABLED` controls the connected-session failed-download worker; defaults to `true`. Its bounded policy is configured with `SLSKR_TRANSFER_AUTO_RETRY_DELAY_SECONDS` (default `1800`), `SLSKR_TRANSFER_AUTO_RETRY_CHECK_INTERVAL_SECONDS` (`300`), `SLSKR_TRANSFER_AUTO_RETRY_MAX_ATTEMPTS` (`5`, or `0` for unlimited), `SLSKR_TRANSFER_AUTO_RETRY_MAX_FILES_PER_CYCLE` (`10`), `SLSKR_TRANSFER_AUTO_RETRY_MAX_FILES_PER_PEER_PER_CYCLE` (`1`), `SLSKR_TRANSFER_AUTO_RETRY_PEER_COOLDOWN_SECONDS` (`900`), `SLSKR_TRANSFER_AUTO_RETRY_ALTERNATE_SOURCES_ENABLED` (`true`), `SLSKR_TRANSFER_AUTO_RETRY_MAX_ALTERNATE_SOURCE_SEARCHES_PER_CYCLE` (`1`), and `SLSKR_TRANSFER_AUTO_RETRY_ALTERNATE_SOURCE_SIZE_TOLERANCE_PERCENT` (`5`). Equivalent TOML keys live under `[transfers.auto_retry]`.
- `SLSKR_TRANSFER_RESCUE_ENABLED` controls the connected-session underperformance detector and defaults to `true`. It considers only audio attempts, matching the sidecar exclusion policy. Queued attempts are eligible after `SLSKR_TRANSFER_RESCUE_MAX_QUEUE_TIME_SECONDS` (`1800`); in-progress attempts are eligible below `SLSKR_TRANSFER_RESCUE_MIN_THROUGHPUT_KBPS` (`10`) after `SLSKR_TRANSFER_RESCUE_MIN_DURATION_SECONDS` (`300`), or after no progress for `SLSKR_TRANSFER_RESCUE_STALLED_TIMEOUT_SECONDS` (`120`). The worker checks every `SLSKR_TRANSFER_RESCUE_CHECK_INTERVAL_SECONDS` (`45`), caps activations with `SLSKR_TRANSFER_RESCUE_MAX_FILES_PER_CYCLE` (`2`), applies `SLSKR_TRANSFER_RESCUE_RETRY_COOLDOWN_SECONDS` (`1800`), and verifies alternate size within `SLSKR_TRANSFER_RESCUE_ALTERNATE_SOURCE_SIZE_TOLERANCE_PERCENT` (`5`). Rescue first derives the sibling-compatible filename/size HashDB key and, when that exact entry has a consistent whole-file SHA-256 plus at least two discovered range peers, downloads to a distinct confined path and promotes it under the stable request identity only after exact size/hash verification. `byteHash` alone is never treated as a whole-file digest. A failed or stale swarm leaves the Soulseek attempt and partial file untouched. The fallback atomically substitutes a cached Soulseek alternate under the same request metadata, or dispatches one bounded global basename search. Cancellation is observed at transfer chunk boundaries before a Soulseek replacement can reuse the destination. Equivalent TOML keys live under `[transfers.rescue]`.
- `SLSKR_POD_JOIN_SIGNATURE_MODE` accepts `off`, `warn`, or `enforce` and defaults to the upstream-compatible `off`. `warn` accepts missing and legacy signatures but strictly verifies any `ed25519:<base64>` signature supplied. `enforce` requires a timestamp within five minutes and a bounded one-use nonce. Join requests remain pending until an authorized signed acceptance, and the matching list/cancel plus privileged leave/accept workflows are bounded in memory. Signed bytes use the frozen pipe-delimited canonical payloads beginning with version `1` and `join-request`, `join-acceptance`, `leave-request`, or `leave-acceptance`; bounded fields containing `|` are rejected to keep field boundaries unambiguous. The TOML equivalent is `[podcore.join].signature_mode`.
- `SLSKR_POD_GOLD_STAR_CLUB_AUTOJOIN` controls the reserved Gold Star Club pod. Native/current slskr launches are opt-in by default; the explicitly selected `legacy` profile keeps its compatibility default-on behavior. The TOML equivalent is `[podcore.gold_star_club].autojoin`. See [Gold Star Club](gold-star-club.md) for its testing/community intent and irreversible local leave behavior.
- `SLSKR_DOWNLOAD_COMPLETED_PATH_TEMPLATE` controls the relative completed-download layout beneath the state download root. Supported tokens are `{uploader}`, `{remote_folder}`, `{remote_parent}`, `{remote_filename}`, `{batch_id}`, `{request_name}`, and `{date}` or `{date:<chrono format>}`. Empty templates preserve the remote path; explicit request destinations take precedence and remain confined.
- `SLSKR_API_TOKEN` is the administrator HTTP API credential and HS256 signing secret for short-lived administrator JWTs issued by `POST /api/v0/session`; five failed source-IP or source-scoped credential attempts trigger a bounded five-minute lockout, and `DELETE /api/v0/session` revokes the presented JWT until expiry. Optional distinct `SLSKR_API_READ_WRITE_TOKEN`, `SLSKR_API_READ_ONLY_TOKEN`, and `SLSKR_API_NOWPLAYING_TOKEN` credentials enforce controller roles and scoped-webhook policies. Protected routes accept `Authorization: Bearer <token>` and automation-compatible `X-API-Key: <token>`; JWT-only and API-key-only controller policies also enforce the presented scheme. Legacy same-site `slskr.session` cookie auth is accepted only when `SLSKR_API_COOKIE_AUTH_ENABLED=true`. Browser clients should keep tokens in memory or session storage rather than long-lived persistent storage.
...
- `SLSKR_PERSISTENCE_ENABLED` enables the default-off SQLite persistence path. Share index, event log, search, transfer, user, browse, message, room, collection, library, destination, now-playing, wishlist, contact, sharegroup, share-grant, user-note, interest, security-ban, OAuth-state, webhook, and runtime compatibility projections write through to SQLite; transfer projection state also remains restart-safe through `transfer-state.json`. Hash and shadow-index discovery metadata is always restart-safe in bounded `content-discovery-state.json` because executable mesh resolution cannot safely fall back to the old synthetic hash projection.
- Spotify integration uses the existing slskr HTTP/WebUI port for OAuth callback handling. Configure `SLSKR_SPOTIFY_ENABLED=true` and `SLSKR_SPOTIFY_CLIENT_ID`; if `SLSKR_SPOTIFY_REDIRECT_URI` is unset, the daemon advertises `http://127.0.0.1:<http-port>/api/integrations/spotify/callback` for loopback use. The callback requires a daemon-issued cryptographically random state value, expires pending state after 10 minutes, rejects replayed, missing, or invalid state, and limits live pending states to 256.
