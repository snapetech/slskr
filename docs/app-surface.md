...
- mesh and service-fabric status projections for DHT, mesh peers, hashes/catalog sequence state, sync/backfill activity, transfer rates, and NAT/overlay health, plus an executable verified multi-source range swarm with pollable jobs. Signed Soulseek capability hello/ack messages populate the bounded live mesh-peer registry. A durable bounded content-discovery store resolves exact size plus `byteHash`, `fullFileHash`, or `fileSha256` metadata to MusicBrainz recording IDs and shadow-index peer IDs; source-less swarm requests then select matching mesh-capable HTTP range endpoints before normal preflight and SHA-256 verification.
