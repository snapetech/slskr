...
- `SLSKR_OBFUSCATED_LISTENER_BIND` to enable and bind the type-1 obfuscated peer listener in the `slskdN` compatibility profile; the frozen `slskd` profile has no type-1 listener
- `SLSKR_OBFUSCATED_ADVERTISED_PORT` for the public obfuscated peer port advertised to the server in the `slskdN` profile
- `SLSKR_OVERLAY_BIND` enables the TLS 1.3 mesh-service listener, for example `0.0.0.0:50305`; the TOML equivalent is `[listeners].overlay_bind`. Enabling it creates durable restricted `overlay-certificate.der`, `overlay-private-key.der`, and `peer-capability-key.bin` identity files in the state directory, advertises `mesh_sync` with the bound port, and exposes the certificate SHA-256 fingerprint through `GET /api/mesh/transport` for Pod policy pinning.
- `SLSKR_DHT_ENABLED` enables Mainline-DHT rendezvous and defaults to enabled when `SLSKR_OVERLAY_BIND` is configured. `SLSKR_DHT_PORT` selects the UDP bind port and defaults to `0` (ephemeral). TOML equivalents are `[dht].enabled` and `[dht].port`. The client refreshes and announces the frozen `slskdn-mesh-v1` primary/backup keys on a bounded interval; `GET /api/dht/status` reports bootstrap, routing, public-address, firewall, announce, lookup, and discovered-beacon state.
- `SLSK_OBFUSCATION` enables type-1 obfuscated listener advertisement and fallback paths in the `slskdN` profile; it defaults to `true` there. The frozen `slskd` profile ignores these slskdN-only obfuscation layers and exposes no type-1 listener. In the `slskdN` profile, `SLSK_OBFUSCATION_MODE=compatibility` is the default and dials regular peer-message, browse, and file-transfer endpoints before obfuscated endpoints. `prefer` reverses that order when `SLSK_OBFUSCATION_PREFER_OUTBOUND=true`; both modes retain the other transport as fallback. Unsupported `only` mode fails startup. Equivalent TOML keys live under `[network.obfuscation]`.
...
- `SLSKDN_POD_GOLD_STAR_CLUB_AUTOJOIN` controls the reserved Gold Star Club pod. Native/current slskR launches are opt-in by default; frozen compatibility launches keep the slskdN-compatible default-on behavior. The TOML equivalent is `[podcore.gold_star_club].autojoin`. See [Gold Star Club](gold-star-club.md) for its testing/community intent and irreversible local leave behavior.
