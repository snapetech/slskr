...
- `SLSKR_POD_JOIN_SIGNATURE_MODE` accepts `off`, `warn`, or `enforce` and defaults to the upstream-compatible `off`. `warn` accepts missing and legacy signatures but strictly verifies any `ed25519:<base64>` signature supplied. `enforce` requires a timestamp within five minutes and a bounded one-use nonce before the join can mutate membership state. Signed bytes are the compact JSON array `[1,"join-request",podId,peerId,requestedRole,timestampUnixMs,message,nonce]`, which unambiguously escapes attacker-controlled fields. The TOML equivalent is `[podcore.join].signature_mode`.
