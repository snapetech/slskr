...
- mesh and service-fabric HTTP status projections, plus an executable verified multi-source HTTP range swarm with pollable jobs. Frozen-format signed Soulseek capability hello/ack messages populate a bounded observed-peer registry. Pinned TLS 1.3 overlay client and server paths implement bounded `SLSKDNM1` framing and the `private-gateway` OpenTunnel/TunnelData/GetTunnelData/CloseTunnel service used by live local port-forwarding rules. Inbound tunnel requests require a fresh Soulseek-authenticated capability, matching peer IP, Pod membership, replay-safe request, and exact private-network destination policy; slskr peers additionally sign the overlay hello with their durable capability key and bind it to the gateway certificate. The BitTorrent-DHT rendezvous is not implemented, so DHT/NAT responses remain compatibility projections and gateway discovery currently uses Soulseek capability plus peer-address observations. A durable bounded content-discovery store resolves exact size plus `byteHash`, `fullFileHash`, or `fileSha256` metadata to MusicBrainz recording IDs and shadow-index peer IDs; source-less swarm requests can select slskr-local HTTP range endpoint metadata before normal preflight and SHA-256 verification.
