...
- mesh and service-fabric HTTP status projections, plus an executable verified multi-source HTTP range swarm with pollable jobs. Frozen-format signed Soulseek capability hello/ack messages populate a bounded observed-peer registry. Pinned TLS 1.3 overlay client and server paths implement bounded `SLSKDNM1` framing and the `private-gateway` OpenTunnel/TunnelData/GetTunnelData/CloseTunnel service used by live local port-forwarding rules. Inbound tunnel requests require a fresh Soulseek-authenticated capability, matching peer IP, Pod membership, replay-safe request, and exact private-network destination policy; slskr peers additionally sign the overlay hello with their durable capability key and bind it to the gateway certificate. Frozen unsigned clients may handshake and ping but cannot invoke private-gateway service methods. A bounded Mainline BitTorrent-DHT client announces and queries the frozen primary and two backup SHA-1 rendezvous keys, publishes real status, and supplies certificate-pinned gateway endpoint candidates when a Soulseek peer address is not cached. A durable bounded content-discovery store resolves exact size plus `byteHash`, `fullFileHash`, or `fileSha256` metadata to MusicBrainz recording IDs and shadow-index peer IDs; source-less swarms can select slskr-local HTTP range endpoint metadata before normal preflight and SHA-256 verification.
...
- `POST /api/v0/virtualsoulfind/shadow-index/sync/merge` and `GET /api/v0/virtualsoulfind/shadow-index/:recordingId`: maintain and inspect the bounded recording-to-peer relation used by mesh source discovery. Source-less `POST /api/v0/multisource/swarm` requests use this relation plus verified live capability descriptors and still require at least two usable range sources.
...
- `POST /api/v0/pods/:podId/channels/:channelId/bind` and `unbind`: persist validated `readonly` or `mirror` Soulseek-room binding metadata for moderators. Live room/DM bridging is not active yet, so these routes do not claim that messages are mirrored.
