...
| `slskdN` adaptive multi-source transfer fixes (`9ca597045`, `c2557f0fd`, `e6a0522db`) | Missing | `slskR` exposes compatibility job projections and can replace a failed transfer with another search source, but it does not yet run one download as a verified chunk swarm across concurrent peers. The upstream empty-peer, empty-average, and non-positive chunk guards therefore identify a missing parent feature, not isolated Rust fixes. |
