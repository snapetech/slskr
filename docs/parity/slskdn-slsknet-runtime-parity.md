...
| `slskdN` private-message gate auto-response (`a87768464`) | Missing | Inbound messages are durably projected and acknowledged, but `slskR` has no bounded opt-in human-check/share-gate classifier, per-user cooldown registry, automatic reply dispatch, configuration surface, or Rust UI toggle. |
