...
| `slskNet.Runtime` legacy peer path encodings (`192c5e4a`) | Implemented/hermetic | Protocol strings now detect and retain UTF-8, Windows-1251, or Latin-1 encoding metadata. Folder requests, search and browse entries, persisted share entries, and transfer requests preserve that metadata when paths are echoed to peers; locally generated paths are explicitly UTF-8. Round-trip and unrepresentable-character regressions cover the wire codecs and peer message families. |
