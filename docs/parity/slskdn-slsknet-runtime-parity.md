...
| `slskdN` wishlist edit/search/download policy (`8a244699a`, `f96ef06a1`, `47bd6c599`, `d91ea7b5c`, `73767889b`, `ac3c08101`, `ffddeff27`, `a1984f8f9`, `9a06ffa8c`) | Partially implemented/missing UI | Filter text, enabled state, auto-download state, maximum results, nullable maximum downloads, viewed/search timestamps, hit counters, totals, and the last search identity are bounded, persisted, rehydrated, and mutable through the API. Incoming results enforce the literal include/exclude filter and per-item result cap, hide locked hits, retain ignored/filtered/locked counters, and expose bounded paged item history. Manual item searches now dispatch to the network; scheduled expirations and explicit completions finalize counters. Auto-download ranks a best peer-directory group, enqueues its files through the normal confined transfer path, records the count, and disables one-shot or limit-reached items. Hermetic regressions cover restart state, filtering, counters, history/viewed routes, best-folder enqueue, and auto-disable. Missing: the equivalent Rust history, unseen-result badge, and policy editor UI. |
