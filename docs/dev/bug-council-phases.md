...
| 6 | Council bughunt entrypoint | Done | (agent) | `scripts/run-council-bughunt.sh` runs fresh candidate counts, process gates, calibrated semantic lenses, and refuses to call the council complete while phases remain pending. |

## How to resume

1. Read this tracker; identify the first non-Done row.
2. Run `scripts/run-council-bughunt.sh`. If it exits with pending phases, pick the first pending row and implement it.
