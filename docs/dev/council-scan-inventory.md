...
| Candidate | Scope | Classification | Severity | Confidence | Evidence | Follow-up |
| --- | --- | --- | --- | --- | --- | --- |
| `read_raw_frame(reader, length)` caller-supplied allocation | Client SDKs + Network Runtime | Fixed | Medium | High | BUG-035: raw frame reads now reject lengths over `DEFAULT_MAX_FRAME_LEN` by default and expose `read_raw_frame_with_max` for stricter callers. | `oversized_raw_frame_is_rejected_before_payload_read`; `scripts/with-build-guard.sh cargo test -p slskr-client`. |
| `ServerConnection::connect` public SDK helper | Client SDKs + Network Runtime | Fixed | Medium | High | BUG-036: default server connects now use `DEFAULT_CONNECT_TIMEOUT`; callers needing tighter policy can call `connect_with_timeout`. | `scripts/with-build-guard.sh cargo test -p slskr-client`; runtime callers already wrap with app-specific timeouts. |
| `connect_peer_messages`, `connect_distributed`, `connect_file_transfer` public SDK helpers | Client SDKs + Network Runtime | Fixed | Medium | High | BUG-036: direct peer/distributed/file-transfer SDK helpers now use `DEFAULT_CONNECT_TIMEOUT` and expose timeout-specific variants. | `scripts/with-build-guard.sh cargo test -p slskr-client`; daemon call sites continue using shorter configured peer timeouts. |
