...
| Resolver/raw stream candidates | 902 |
| Task/cancellation/lifecycle candidates | 1008 |
| Example Web API candidates | 276 |
...
| Daemon direct peer/file connect call sites | Backend/API + Network Runtime | Existing Guard | Low | High | `crates/slskr/src/lib.rs` wraps direct and indirect peer/file `TcpStream::connect`, handshake, read, write, and transfer operations in `state.config.peer_response_timeout`. | Existing daemon peer/transfer tests plus client tests. |
