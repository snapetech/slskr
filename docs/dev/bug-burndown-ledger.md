...
| BUG-020 | Client SDKs | Go/Python CI coverage | Medium | High | CI/release gate do not install Go or run Go/Python client tests. | SDK compile/runtime regressions can ship. | Client SDKs + Release/Ops | Pinned Go/Python setup plus `go test ./...` and Python checks in CI/release. | Accepted |
