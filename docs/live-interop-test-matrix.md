...
| `slskr` live `slskd_api` automation client smoke | Passed | 2026-05-05 local daemon run with Python `slskd_api` 0.2.4: `scripts/run-slskd-api-compat-smoke.sh` passed 91 client calls across application, session, server, search, transfer, room, conversation, user, file, relay, share, options, events, logs, and telemetry APIs |
