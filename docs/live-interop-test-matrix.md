...
| Peer metadata | Server returns regular and obfuscated listener metadata | `probe peer-address` | `slskr -> slskr`, `slskr -> slskr` address probes |
| Plain peer messaging | Plain `PeerInit` and `UserInfoRequest` round-trip | `probe plain-peer` | `slskr -> slskr`, `slskr -> slskr` plain probes |
| Obfuscated peer messaging | Type-1 obfuscated `PeerInit` and `UserInfoRequest` round-trip | `probe obfuscated-peer` | `slskr -> slskr` attempted when the daemon advertises/runtime-enables it |
...
| File-transfer init | Transfer `F` peer init is accepted only as part of a real queued transfer | `probe file-transfer-peer` against `slskr` self-smoke | Raw token-echo closes with EOF against `slskr`/`slskr` as expected; queued payload transfer proof now covers daemon bytes |
...
| restart/persistence matrix | Passed over VPN-isolated daemon/probe namespaces | 2026-05-04 restart runs: `slskr` and `slskr` restarted with preserved app/share directories, republished peer metadata, served browse responses, completed open-commons queued payload downloads, and completed search soaks |
...
| current `slskr <-> slskdN` cross-client matrix | Passed, 56/56 functional checks plus soak | 2026-07-16 isolated credentialed run against the current slskdN checkout: both clients completed VirtualSoulfind v2 track-intent creation, pending listing, empty planning, release-intent creation, atomic processing, terminal failure, and exact-once statistics. Both-direction browse, search, exact-hash downloads, private messages, queued FLAC-header backfill, signed `KSDN` registry exchange, pinned TLS 1.3 `dht.Ping`, signed `dht.Store`, and mesh/ticket checks also passed. slskr resolved slskdN's real shared-file content ID and fetched its exact 42-byte FLAC fixture through `MeshContent/GetByContentId`, validating the raw response SHA-256. The pinned `pods` service completed List/Get/Join/PostMessage/GetMessages/Leave against a Pod created in slskdN. A second Pod designated slskdN's signed local profile as its gateway; slskr joined it, opened a policy-authorized private TCP tunnel, sent an exact payload to a private echo server, received the exact bytes, closed the tunnel, and passed the 30-second soak. Evidence: `target/live-interop/slskr-slskdn-cross-client-interop.tsv` (57 TSV rows including the soak). |
...
| VirtualSoulfind v2 activation | slskdN's v2 controller injected a separate unbound options type whose default disabled the API even though the configured processor/backends were enabled. Its hard-coded `/api/v1` route also still requires an explicit API-version query. | Bind the controller monitor to the root VirtualSoulfind v2 options and make the cross-client probe send `api-version=1`. Both clients now pass the same bounded create/plan/process/statistics workflow. |
| Cross-client harness accuracy | Mixed auth headers blocked slskR readiness, compilation after slskdN startup consumed its endpoint-override cache lifetime, the slskR download assertion used a Windows path on Linux, mesh health omitted its required API version, and Bash conditional errexit recorded an HTTP 400 as `ok`. | Use one bearer credential only for slskR and no credential for the auth-disabled slskdN fixture, build before launching daemons, check the normalized Linux path, request mesh health with `api-version=1.0`, and explicitly reject failed or malformed mesh responses. |
| Capability and overlay discovery | slskR only answered `KSDN` messages; its active rendezvous flag was never enabled or wired to a network probe. The overlay certificate is created lazily, so hashing an absent file produced the SHA-256 of empty input instead of a usable pin. | Active discovery and explicit mesh sync now queue bounded signed probes, enforce acknowledgement type and nonce, verify the descriptor, and persist it. The harness first triggers lazy certificate creation, then extracts and enforces the actual DER fingerprint before the frozen overlay service call. |
| Backfill compatibility | slskR exposed candidate-count placeholders but lacked slskdN's backfill controls, scheduler, peer limits, and FLAC-header probe. | Added the sibling-shaped control routes, idle scheduler, bounded restart-safe daily counters, two-probe concurrency cap, exact FLAC/32 KiB SHA-256 policy, durable HashDB merge, and queued upload-token handling. The current-client matrix proves the live slskR-to-slskdN header exchange and byte hash. |
| Private-gateway production path | slskdN registered neither the Pod/gateway overlay services nor a usable production gateway identity, treated a committed Pod message as failed when best-effort fan-out failed, and re-resolved approved tunnel hostnames while dialing, creating a DNS-rebinding TOCTOU gap and mapped-address false rejection. | Register all mesh services, retain committed-message success independently of fan-out, bind gateway authorization to the signed local profile, dial only the already validated IP addresses, and compare parsed IP addresses semantically. Focused tests and the live exact-byte tunnel workflow pass. |
...
| Restart log diagnostics | slskr restart logs contained null padding and HTTP readiness relied on logs because the daemon bound HTTP to loopback inside the namespace. | TSV sanitization now strips null bytes, and daemon launches set `ASPNETCORE_URLS=http://0.0.0.0:$port` for reachable HTTP readiness where the target honors ASP.NET Core URL binding. |
...
No blocking implementation gaps remain in the current live interop matrix. The raw `file-transfer-peer` row remains diagnostic-only because `slskr` and `slskr` correctly require an actual queued transfer before opening `F` payload sockets; queued payload transfer is covered by `download-peer` and open-commons download probes.
