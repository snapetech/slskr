...
- CI: `scripts/check-rust-format.sh`, `cargo clippy -D warnings`, `cargo test`.
...
- Live interop. *(done current cut for slskr-to-slskr: VPN-backed live matrix proves login, direct peer-message, obfuscated peer-message with compatibility fallback, indirect peer-message, browse, private message, room message, search soak, queued payload transfer, and restart/persistence paths; third-party type-1 obfuscated peer proof remains optional because another reachable implementation must advertise type `1`)*
...
- Public provenance/branding scrub. *(done current cut: `scripts/check-public-posture.sh` now scans README, plan, compliance/notice text, package metadata, crates, clients, web, docs, k8s, and workflows for public-origin or affiliation claims; compliance/prohibition text is allowlisted so the gate catches claims rather than warnings)*
- Bundled app crate. *(done current cut: `crates/slskr` is the product binary, owns `slskr serve`, smoke/probe subcommands, HTTP API, WebSocket event feed, background session manager, listener ownership, share indexing, browse/search/message/room/user/transfer projections, direct and indirect peer/file-transfer execution, SQLite-backed app stores, auth/rate-limit/CSP/public-posture guards, live matrix proof, and release package checks; remaining work is production hardening rather than app scaffolding)*
...
- App surface runbook. *(done current cut: see `docs/app-surface.md` for command layout, HTTP shell, auth defaults, config direction, persistent TOML example, packaging target, and remaining production-hardening list)*
...
- Public repo/package rename. *(permission-gated/deferred: do not rename repository, packages, crates, namespaces, images, or user-visible app identifiers without explicit maintainer approval)*
- 24h soak test (search + transfer + distributed-tree health). *(harness exists and focused VPN-backed live/proton matrices passed; rerun `scripts/run-live-soak-24h.sh` before release tagging when credentials and Proton namespaces are intentionally available)*
...
- `SLSKR_INDIRECT_HOST_OVERRIDE=127.0.0.1 cargo run -p slskr --bin slskr -- smoke local-peer`
- `cargo run -p slskr --bin slskr -- soak live` with a bounded 10 second window
